...
.PHONY: all clean host bootloader kernel drivers services apps tools docs serve \
        test c-tests py-tests ts-tests iso iso-full iso-quick installer initramfs release check-env \
...
tools:
        ...
        @chmod +x tools/add_module_section.py tools/gen_wli.py tools/serve.py tools/build_iso.sh tools/installer.sh tools/create_initramfs.sh 2>/dev/null || true
...
# ISO build (bootable memory OS) — same as iso-full
iso: iso-full

# Full ISO build: builds everything first, then creates bootable ISO
iso-full: all initramfs
        @echo "[ISO] Building full bootable ISO image..."
        chmod +x tools/build_iso.sh 2>/dev/null || true
        ./tools/build_iso.sh dist/webos-$(VERSION).iso

# Quick ISO build: skips dependency checks, data-only or partial
iso-quick:
        @echo "[ISO] Building quick ISO image (no kernel/initramfs checks)..."
        chmod +x tools/build_iso.sh 2>/dev/null || true
        ./tools/build_iso.sh dist/webos-$(VERSION).iso --quick

# Build initramfs only
initramfs: tools
        @echo "[Initramfs] Building minimal initramfs..."
        chmod +x tools/create_initramfs.sh 2>/dev/null || true
        sudo ./tools/create_initramfs.sh build/initrd.img

# Build installer media (ISO with installer as default boot)
installer: iso-full
        @echo "[Installer] Installer is included in the ISO."
        @echo "  Boot the ISO and select 'WebOS Installer' from GRUB menu."
        @echo "  Or run: sudo ./tools/installer.sh /dev/sdX"

# Release package
release: all test iso-full
...
# Clean ISO artifacts only
clean-iso:
        rm -rf build/iso build/initrd.img dist/webos-$(VERSION).iso
...
help:
        ...
        @echo ""
        @echo "  ISO / Installer Targets:"
        @echo "    iso          Build bootable ISO (alias for iso-full)"
        @echo "    iso-full     Full ISO build with all dependencies"
        @echo "    iso-quick    Quick ISO build (no kernel check, data-only)"
        @echo "    initramfs    Build minimal initramfs only"
        @echo "    installer    Build installer media"
        ...
        @echo "    clean-iso    Remove ISO build artifacts only"
